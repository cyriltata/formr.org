...
#		pr($this->id);
		...
		while (!$output): // only when there is something to display or if 80 tries have been made, stop.
			$i++;
			if ($i > 80) {
				global $user;
			...
			if ($unit_info) { // if there is one, spin that shit 
				...
				$output = $unit->exec(); // In which cases will this not produce output? or at least return true?
				...
				// if there is nothing in line yet, add the next one in run order
				if (!$this->runToNextUnit()) {
	...
	public function getUnitIdAtPosition($position) {
		$data = $this->dbh->prepare("SELECT unit_id FROM `survey_run_units` WHERE run_id = :run_id AND position = :position LIMIT 1");
	...
	public function runTo($position, $unit_id = null) {
		...
		if ($unit_id):
			...
			if ($unit_session->id):
				$run_to_q = "UPDATE `survey_run_sessions` SET position = :position  WHERE id = :id LIMIT 1;";
	...
	public function runToNextUnit() {
		$g_unit = $this->dbh->prepare(
				"SELECT unit_id, position FROM `survey_run_units` WHERE  run_id = :run_id AND position > :position ORDER BY position ASC LIMIT 1 ;");
